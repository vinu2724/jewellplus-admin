...
const useUpdateCRM = () => {
  ...
  const updateCRM = async (
    ...
    if (!searchCode || !updateCRM) return null; // Ensure required fields are provided

    setLoading(true);
    setError(null);

    try {
      const response = await fetch("/api/update-crm", {
        method: "POST",
  ...
  return { updateCRM, error, loading };
};
