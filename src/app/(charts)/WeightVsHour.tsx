...
const WeightVsHour: React.FC<Props> = ({
  title,
  salesData,
  loading,
  error,
  ...
  if (!salesData?.length) return <p>No sales data available.</p>;

  const itemTypeDetails: Record<
    string,
  ...
  salesData.forEach((item) => {
    ...
    if (hour !== null && hour >= 0 && hour <= 23 && itemTypeDetails[itemType]) {
      itemTypeHourMap[itemType][hour] += weight;
    }
  });
  ...
  const options = {
    ...
    scales: {
      x: {
        title: { display: true, text: "Hour (12AM - 11 PM)" },
      },
      y: {
        beginAtZero: true,
      ...
      <div className="bg-white shadow-lg rounded-2xl p-4 sm:p-6 w-full h-[500px] sm:h-[600px] lg:h-[450px] relative">
        ...
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-lg sm:text-xl font-semibold text-gray-800 flex-1 text-center pr-2">
            {title || "Hourly Sales Weight by Item Type"}
          </h3>
