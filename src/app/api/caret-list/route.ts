...
export async function POST(request: NextRequest) {
  try {
    ...
    const API_URL = API_ENDPOINTS.CARET_LIST;

    if (!API_URL) {
      console.error(
        "ERROR : process.env.NEXT_PUBLIC_CARET_LIST is not defined in the environment variables."
      );
    ...
    console.error("Error fetching Caret list:", error);
    return NextResponse.json(
